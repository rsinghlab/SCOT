...
X=np.loadtxt("s1_mapped1.txt")
y=np.loadtxt("s1_mapped2.txt")
X=zscore_standardize(X)
y=zscore_standardize(y)
print(X.shape, y.shape)

Xlabel=np.loadtxt("s1_type1.txt")
ylabel=np.loadtxt("s1_type2.txt")

uc=UnionCom.UnionCom()
integrated_data=uc.fit_transform(dataset=[X,y])
np.save("simu1UC_norm.npy", integrated_data)
Xn=integrated_data[0]
yn=integrated_data[1]
print(Xn.shape, yn.shape)
print("FOSCTTM", np.mean(calc_domainAveraged_FOSCTTM(Xn, yn)))
